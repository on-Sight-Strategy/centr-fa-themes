...
[@helpers.html]
  [@helpers.head title="Centr | Registration Verification"]
    [@helpers.captchaScripts showCaptcha=showCaptcha captchaMethod=tenant.captchaConfiguration.captchaMethod siteKey=tenant.captchaConfiguration.siteKey/]
  [/@helpers.head]
  [@helpers.body]
    [#-- VDB: Verification uses mainBoost with full background image --]
    [@helpers.mainBoost title=theme.message('registration-verification-form-title')!"VERIFY YOUR REGISTRATION." subtitle="" rowClass="row center-xs" colClass="col-xs col-sm-8 col-md-6 col-lg-5 col-xl-4" showCoverImage=true showHeader=false titleClass="text-white"]
      [#-- FusionAuth automatically handles errors that occur during registration verification and outputs them in the HTML --]
      <form action="${request.contextPath}/registration/verify" method="POST" class="flex flex-col flex-grow justify-start w-full gap-y-4">
